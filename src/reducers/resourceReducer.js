...
const startUxBlockingApiCall = (action, currentState) => {
  action.followupAction(appActions.blockUx(action.source))
  return startApiCall(action, currentState)
}

const completeUxBlockingApiCall = (action, currentState) => {
  action.followupAction(appActions.releaseUx(action.source))
...
const resourceReducer = (currentState = INITIAL_STATE, action) => {
  ...
  switch (action.type) {
    ...
    case resourceActions.DELETE_ITEM_REQUEST:
      return startUxBlockingApiCall(action, currentState)
    ...
    case resourceActions.DELETE_ITEM_FAILURE:
      return completeUxBlockingApiCall(action, currentState)
    // item event handlers
